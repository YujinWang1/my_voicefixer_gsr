...
class SSR_UNet(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
       ...
       # return {"loss": val_loss}
        ...
        sp_low_quality, _ = self.pre(low_quality)
        estimation = self(sp_low_quality, low_quality)['wav']
        # 直接用波形计算损失（f_loss 内部会转为频谱图）
        val_loss = self.f_loss(estimation, vocal)  # 关键：传入波形
        self.log("val_l", val_loss, on_step=False, on_epoch=True, logger=True, sync_dist=True, prog_bar=True, batch_size=1) ##先把波形转换为频谱图 再进行进一步的修改
        return {"loss": val_loss}