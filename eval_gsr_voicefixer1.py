    ...
    with torch.no_grad():
        ...
        while break_point < wav_10k.shape[0]+seg_length:
            ...
            try:
                ...
                print(f"Error during vocoder decoding: {e}")
            ...
            if out is not None:
                ...
                print("Warning: 'out' is not defined. Skipping this segment.")    

             # ======== 时域指标 ========
            # 降采样到 16kHz 以符合 PESQ、STOI 要求
            sr_target = 16000
            out_16k = torchaudio.functional.resample(
                out.cpu(), orig_freq=44100, new_freq=sr_target
            ).numpy().squeeze()

            tgt_16k = torchaudio.functional.resample(
                torch.tensor(target_segment).unsqueeze(0), orig_freq=44100, new_freq=sr_target
            ).numpy().squeeze()
            print("tgt_16k shape:", tgt_16k.shape)
            print("out_16k shape:", out_16k.shape)
            # PESQ: wideband mode (wb)
            try:
                pesq_score = pesq(sr_target, tgt_16k, out_16k, 'wb')
            except Exception as e:
            ...
            # STOI
            try:
                stoi_score = stoi(tgt_16k, out_16k, sr_target, extended=False)
            except Exception as e:
                print("STOI calculation failed:", e)
                stoi_score = None

            metrics["pesq_wb"] = float(pesq_score) if pesq_score is not None else None
            metrics["stoi"] = float(stoi_score) if stoi_score is not None else None
