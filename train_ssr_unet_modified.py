import git
import sys
import os
import torch
import pytorch_lightning as pl

# Locate project root via Git
git_root = git.Repo("", search_parent_directories=True).git.rev_parse("--show-toplevel")
sys.path.append(git_root)

from pytorch_lightning import Trainer
from pynvml import *
from pytorch_lightning.callbacks import ModelCheckpoint, LearningRateMonitor
from pytorch_lightning.loggers import CSVLogger
from pytorch_lightning.plugins.training_type.ddp import DDPPlugin
from pytorch_lightning.callbacks.progress import TQDMProgressBar
from dataloaders.data_module import SrRandSampleRate
from tools.callbacks.base import *
from tools.callbacks.verbose import *

import tools.utils
from tools.dsp.lowpass import *
from models.ssr_unet import SSR_UNet

if not torch.cuda.is_available():
    raise RuntimeError("CUDA is required to run this script.")


from omegaconf import DictConfig, ListConfig

def to_plain(obj):
    """Recursively convert omegaconf objects to plain Python types."""
    if isinstance(obj, (DictConfig, ListConfig)):
        return obj.to_dict()
    return obj


hp, parser = tools.utils.get_hparams()

assert hp["data"]["sampling_rate"] == 44100
hp["root"] = git_root


train_dataset = to_plain(hp["data"]["train_dataset"])
val_dataset   = to_plain(hp["data"]["val_dataset"])
# print("Vocals dataset path:", hp["data"]["train_dataset"]["vocals"]["vctk"])

if isinstance(train_dataset, dict):
    for dataset_name, dataset_params in train_dataset.items():
        if isinstance(dataset_params, dict):
            for param_key, param_value in dataset_params.items():
                hp["data"]["train_dataset"][dataset_name][param_key] = os.path.join(
                    hp["root"], param_value
                )

if isinstance(val_dataset, dict):
    for dataset_name, dataset_params in val_dataset.items():
        if isinstance(dataset_params, dict):
            for param_key, param_value in dataset_params.items():
                hp["data"]["val_dataset"][dataset_name][param_key] = os.path.join(
                    hp["root"], param_value
                )

hp["augment"]["params"]["rir_root"] = os.path.join(hp["root"], hp["augment"]["params"]["rir_root"])

parser = pl.Trainer.add_argparse_args(parser)
args = parser.parse_args()

nvmlInit()
gpu_nums = int(nvmlDeviceGetCount())
distributed = gpu_nums > 1

logger = CSVLogger(os.path.dirname(hp.model_dir), name=os.path.basename(hp.model_dir))
hp["log_dir"] = logger.log_dir

model = SSR_UNet(hp, channels=1, type_target="vocals")
dm = SrRandSampleRate(hp, distributed)

callbacks = [
    LearningRateMonitor(logging_interval='step'),
    ModelCheckpoint(
        filename='{epoch}-{step}-{val_l:.2f}',
        dirpath=os.path.join(logger.log_dir, "checkpoints"),
        save_top_k=hp["train"]["save_top_k"]
    ),
    initLogDir(hp, current_dir=os.getcwd()),
    TQDMProgressBar(refresh_rate=hp["log"]["progress_bar_refresh_rate"])
]

trainer = Trainer.from_argparse_args(
    args,
    gpus=gpu_nums,
    strategy=DDPPlugin(find_unused_parameters=False) if torch.cuda.is_available() else None,
    max_epochs=hp["train"]["max_epoches"],
    detect_anomaly=True,
    num_sanity_val_steps=2,
    resume_from_checkpoint=hp["train"]["resume_from_checkpoint"] if hp["train"]["resume_from_checkpoint"] else None,
    callbacks=callbacks,
    sync_batchnorm=True,
    replace_sampler_ddp=False,
    check_val_every_n_epoch=hp["train"]["check_val_every_n_epoch"],
    logger=logger,
    log_every_n_steps=hp["log"]["log_every_n_steps"]
)

dm.setup('fit')
trainer.fit(model, datamodule=dm)
